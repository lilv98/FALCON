...
def extract_nodes(cfg, filename):
    ...
    return pd.DataFrame(tbox_name_list, columns=['h', 'r', 't']), tbox_desc, tbox_all, all_concepts, all_relations
...
def read_abox_ee(cfg):
    ret = []
    # ['INTERACTOR_A', 'INTERACTOR_B', 'OFFICIAL_SYMBOL_A', 'OFFICIAL_SYMBOL_B', 'ALIASES_FOR_A', 'ALIASES_FOR_B', 
    # 'EXPERIMENTAL_SYSTEM', 'SOURCE', 'PUBMED_ID', 'ORGANISM_A_ID', 'ORGANISM_B_ID']
    with open(cfg.data_root + 'BIOGRID-ALL-4.4.211.tab.txt') as f:
        for line in f:
            line = line.strip().split('\t')
            if len(line) == 11:
                ret.append([line[2], 'interactWith', line[3]])
    ret = pd.DataFrame(ret, columns=['h', 'r', 't']).drop_duplicates()
    return ret

def read_abox_ec(cfg):
    ret = []
    # ['#Format: entrez-gene-id', 'entrez-gene-symbol', 'HPO-Term-ID', 'HPO-Term-Name', 
    # 'Frequency-Raw', 'Frequency-HPO', 'Additional Info from G-D source', 'G-D source', 'disease-ID for link']
    with open(cfg.data_root + 'genes_to_phenotype.txt') as f:
        for line in f:
            line = line.strip().split('\t')
            if len(line) > 1:
                ret.append([line[1], 'hasPhenotype', line[2]])
    ret = pd.DataFrame(ret, columns=['h', 'r', 't']).drop_duplicates()
    ret.t = ret.t.map(lambda x: '<http://purl.obolibrary.org/obo/' + x.split(':')[0] + '_' + x.split(':')[1] + '>')
    return ret
...
def get_data(cfg):
    tbox_name_train, tbox_desc_train, tbox_all, all_concepts, tbox_relations_train = extract_nodes(cfg, filename='TBox.txt')
    tbox_test_pos, all_concepts_test = read_tbox_test(cfg, filename='TBoxInferred.txt')
    tbox_all = tbox_all + tbox_test_pos
    tbox_test_pos = tbox_test_pos[:1000]
    assert all_concepts == (all_concepts_test | all_concepts)
    try:
        tbox_test_neg = []
    ...
    all_entities = set(abox_ec.h.unique()[:cfg.n_e]) 
    ...
    try:
        ...
        print('New GGI Split!', flush=True)
        abox_ee_test = abox_ee[np.random.rand(len(abox_ee)) < 0.2].sample(frac=1)
    ...
    return tbox_name_train, tbox_desc_train, tbox_test_pos, tbox_test_neg, abox_ec, abox_ec_created, abox_ee_train, abox_ee_test, c_dict, e_dict, e_dict_more, r_dict, already_ts_dict, already_hs_dict
...
def parse_args(args=None):
    ...
    # Untunable
    parser.add_argument('--data_root', default='../../data/HPO/', type=str)
